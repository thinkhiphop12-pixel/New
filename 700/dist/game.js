...
import { createDraftState, applySpin, applyPick, applyReroll, canPickCard, isComplete, lineupBySlot, getAvailableRerolls, remainingNeeds, rerollOptions, DEFAULT_RULES, } from './engine.js';
...
function pickCard(card) {
    if (!canPickCard(currentDraft, card))
        return alert("Can't pick this card");
...
function reroll(kind) {
    ...
    if (available.length === 0)
        return alert('No more rerolls available');
...
function renderUI() {
    ...
    rerollOptions(currentDraft, currentSquad, getAllSquads()).forEach(opt => {
        ...
        if (opt.enabled && !opt.used)
            b.onclick = () => reroll(opt.kind);
